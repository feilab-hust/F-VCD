...
def MultiRes_UNet(n_interp,sub_pixel,lf_extra, n_slices, output_size, is_train=True, reuse=False, name='unet',**kwargs):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope('interp'):
            for i in range(n_interp):
                channels_interp = channels_interp / sub_pixel
                n = SubpixelConv2d(n, scale=sub_pixel, name='interp/subpixel%d' % i)
        ...
        #pyramid_channels = [256, 512, 512, 1024, 1024, 1024]
        ...
        with tf.variable_scope('decoder'):
            ...
            for idx in range(nl - 1, -1, -1): # idx = 4,3,2,1,0
                ...
                # n = DropoutLayer(n, keep=0.5, is_fix=True, is_train=is_train, name='dropout%d' % (nl - idx + 1))
...
def MultiRes_UNetX5_crossDynamic(lf_extra, n_slices, output_size, is_train=True, reuse=False, name='unet',**kwargs):
    n_interp = 2 #2 for scan6, 3 for scan6
    if 'channels_interp' in kwargs:
        channels_interp = kwargs['channels_interp']
    else:
        channels_interp=128
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope('decoder'):
            ...
            for idx in range(nl - 1, -1, -1): # idx = 4,3,2,1,0
                ...
                with tf.variable_scope('res_path_%d'%(nl - idx)):
                    out= en_layer
                    for i in range(nl-idx):
                        short_cut=out
                        res_en_layer_1x1= conv_block(short_cut,n_filter=out_channels,kernel_size=1,activation=tf.identity,name='res_%d_1'%(i))
                        res_en_layer_3x3 = conv_block(short_cut, n_filter=out_channels, kernel_size=3, name='res_%d_2'%(i))
                        res_en_lyaer_1_m=merge([res_en_layer_1x1,res_en_layer_3x3],name='res_M%d'%i)
                        res_en_lyaer_1_m = LReluLayer(res_en_lyaer_1_m, name='res_l%d'%i)
                        out = instance_norm(res_en_lyaer_1_m, is_train=is_train, name='res_in%d'%i)

                n = ConcatLayer([out, n], concat_dim=-1, name='concat%d' % (nl - idx))
                n = conv2d(n, out_channels, filter_size=3, stride=1 ,name='conv%d' % (nl - idx + 1))
                n = LReluLayer(n, name='relu%d' % (nl - idx + 1))
                n = instance_norm(n, is_train=is_train, name='in%d' % (nl - idx + 1))
                n = tl.layers.UpSampling2dLayer(n ,size=out_size ,is_scale=False, name = 'upsamplimg%d' % (nl - idx + 1))
