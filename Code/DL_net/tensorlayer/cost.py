...
def mean_squared_error(output, target, is_mean=False, name="mean_squared_error"):
    ...
    with tf.name_scope(name):
        ...
        elif output.get_shape().ndims == 5:  # [batch_size, w, h, c]
            if is_mean:
...
def absolute_difference_error(output, target, is_mean=False):
    ...
    """
    with tf.name_scope("absolute_difference_error"):            #源代码该名称
        if output.get_shape().ndims == 2:  # [batch_size, n_feature]
            if is_mean:
            ...
            if is_mean:
                loss = tf.reduce_mean(tf.reduce_mean(tf.abs(output - target), [1, 2, 3,4]))
            else:
                loss = tf.reduce_mean(tf.reduce_sum(tf.abs(output - target), [1, 2, 3,4]))
        else:
            raise Exception("Unknow dimension")
        return loss
